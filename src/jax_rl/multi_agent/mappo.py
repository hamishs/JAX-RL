...
class MAPPO(BaseAgent):
	''' Multi-agent proximal Policy optimisation for discrete action spaces using
	shared actor parameters and an omniscient critic.'''

	def __init__(self, key, n_states, n_actions, n_agents, gamma, lmda, epsilon,
		v_weight, e_weight, buffer_size, policy, value, lr_policy, lr_value):
		super(MAPPO, self).__init__(key, n_states, n_actions, gamma)
	...
	def act(self, s, return_prob=False, exploration=True):
		...
		for i in range(self.n_agents):
			if exploration:
				...
				a = int(jnp.argmax(p[i]))
			actions.append(a)
			prob.append(p[i,a])

		if return_prob:
			return actions, prob[a]
		else:
			return actions
